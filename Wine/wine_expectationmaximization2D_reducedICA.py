...
#     plt.tight_layout()
#     #plt.show()
#     plt.savefig('EM 2D clustering - ICA Reduced.png', bbox_inches='tight')
...
with plt.style.context('seaborn-whitegrid'):
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('EM 2D clustering - 2 components - ICA Reduced.png', bbox_inches='tight')

Stop_Timer(start_time)

######
# Run em clustering with 3 clusters
######
cluster = GaussianMixture(random_state=0, n_components=3).fit(X_toCluster)
cluster_labels = cluster.predict(X_toCluster)
...
######
# Run em clustering with 15 clusters
######
cluster = GaussianMixture(random_state=0, n_components=15).fit(X_toCluster)
cluster_labels = cluster.predict(X_toCluster)
