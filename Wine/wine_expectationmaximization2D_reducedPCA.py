...
projection = ProjectionAlgorithm(n_components=10)
...
#     plt.tight_layout()
#     #plt.show()
#     plt.savefig('EM 2D clustering - PCA Reduced.png', bbox_inches='tight')
...
with plt.style.context('seaborn-whitegrid'):
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('EM 2D clustering - 2 components - PCA Reduced.png', bbox_inches='tight')

Stop_Timer(start_time)

######
# Run em clustering with 17 clusters
######
cluster = GaussianMixture(random_state=0, n_components=17).fit(X_toCluster)
cluster_labels = cluster.predict(X_toCluster)
...
######
# Run em clustering with 100 clusters
######
cluster = GaussianMixture(random_state=0, n_components=100).fit(X_toCluster)
cluster_labels = cluster.predict(X_toCluster)
